...
class Dipole(object):
    ...
    def get_mag_field(self, target):
        ...
        if not len(target)==len(self.location):
            raise ValueError("Dimensions of arrays do not match")
        mag_perm = 10**-7  # Cancel constant terms to get this
        relative_loc = np.subtract(np.array(target), self.location)
        magnitude = func.get_mag(relative_loc)
        return mag_perm * (
                (3*relative_loc*(np.dot(self.moment, relative_loc))
                / (magnitude**5)
                )
...
def create_dipole_grid(x_array, y_array, strength=1e-3,
    ...
    for i, x in enumerate(dipole_x_pos):
        for j, y in enumerate(dipole_y_pos):
            ...
            #print(i, j)
            dipole_grid.change_point((i, j),
                                    Dipole(orientation=angle,
                                    location=[x, y, 0],
                                    strength=strength))
    mid = time.time()
